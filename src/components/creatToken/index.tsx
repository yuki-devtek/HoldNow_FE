...
import { FinalTokenPoolData, SellTaxDecayData, StageDurationData, StagesData } from "@/config/TextData";
  ...
  return (
    <div className="w-full mx-auto px-3 pb-16">
      <div className="w-full flex flex-col gap-3 mx-4">
        <div onClick={() => handleToRouter("/")} className="cursor-pointer text-white text-2xl flex flex-row items-center gap-2 pb-2">
          <IoMdArrowRoundBack />
          Back
        </div>
      ...
      <div className="w-full h-full justify-between items-start flex flex-col sm2:flex-row sm2:gap-10">
        ...
        <div className="w-full flex flex-col gap-4 sm2:py-5">
          <div className="flex flex-col gap-4 sm2:pt-6">
            <SelectInput header="Final Token Pool Destination" data={FinalTokenPoolData} setSelectData={(inputData) => setTokenPollDestination(inputData.id)} style="h-[162px]" firstData="NEWLP / SOL" />
            <ImageUpload header="Airdrop List (Optional) - CSV Upload" setFilePreview={(fileName) => setCsvFilePreview(fileName)} setFileUrl={(fileUrl) => setCsvFileUrl(fileUrl)} type=".csv" />
          </div>
          ...
          <ImageUpload header="FP Banner Image (Optional)" setFilePreview={(fileName) => setBannerImagePreview(fileName)} setFileUrl={(fileUrl) => setBannerImageUrl(fileUrl)} type="image/*" />
        </div>
      </div>
      <button
        onClick={createCoin}
